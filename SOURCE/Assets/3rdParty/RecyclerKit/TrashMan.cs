...
public partial class TrashMan : MonoBehaviour
{
    ...
    /// <returns></returns>
    public static T spawn<T>(string gameObjectName, Vector3 position = default( Vector3 ), Quaternion rotation = default( Quaternion ), bool bForceBehaviour = false, Vector3 vLocalRotation = default(Vector3))
    {
        GameObject pGO = spawn(gameObjectName, position, rotation);
        if (pGO == null)
            return default(T);
        pGO.transform.localEulerAngles = vLocalRotation;
        var pObj = pGO.GetComponent(typeof(T));
        // force add compoenents
        if(pObj == null && bForceBehaviour)
            pObj = pGO.AddComponent(typeof(T));
        // return if found
        if (pObj != null)
            return (T) Convert.ChangeType(pObj, typeof(T)); ;
        return default(T);
    }
	/// <summary>
